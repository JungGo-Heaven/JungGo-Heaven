...
public class ChatMessageServiceTest {
    ...
    public void 채팅메시지_생성_채팅방_있을때() {
        ...
        verify(redisService, times(1)).saveChatMessageToZSet(any(RedisChatMessageDto.class));
    ...
    public void 채팅메시지_생성_채팅방_없을때() {
        // given
        ChatMessageRequestDto requestDto = new ChatMessageRequestDto(null, 1L, "test message");
        when(productFinder.findProductById(anyLong())).thenReturn(product);
        when(chatRoomWriter.save(any(ChatRoom.class))).thenReturn(chatRoom);
    ...
    public void 읽지_않은_메시지_읽음처리() {
        // given
        ChatMessage chatMessage = ChatMessage.of(1L, chatRoom, buyerUser, "test message", MessageType.TEXT, LocalDateTime.now(), false);
    ...
    public void 메시지_삭제_권한없음() {
        ...
        ChatMessage message = ChatMessage.of(1L, chatRoom, buyerUser, "test message", MessageType.TEXT, LocalDateTime.now(), false);
